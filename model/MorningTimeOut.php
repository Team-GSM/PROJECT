<?php
	...
	if (isset($_POST['morningTimeOut-btn'])) {
				...
				$sql = "SELECT * FROM micp_attendance_table WHERE studentId = '$studentId'";
					...
					if ($ResultCheck > 0) {
						...
						if ($row = mysqli_fetch_assoc($result)) {
								...
								$sql = "UPDATE micp_attendance_table SET morningTimeOut = '$time' WHERE studentId = '$studentId'" ;
								mysqli_query($conn,$sql);
									...
									if ($row = mysqli_fetch_assoc($resultProfile)) {
											...
											header("Location: ../view/User_Page.php?morningTimeOut=success&name=".$row['micp_firstname']."");
										}
